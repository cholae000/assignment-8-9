...
void phoneno_validation(unsigned int phone);
...
void login(){
    ...
    while (existEmail == -1 || charArray==-1){
        ...
        compare_two_charArray(db[existEmail].password,l_pass);

        if(existEmail == -1 || charArray==-1){
            existEmail=-1;
            charArray=-1;
...
void userSector(){
    ...
    if(user_option==1){
        ...
        while (charArray==-1){
            ...
            scanf(" %[^\n]",&tran_pass[0]);
            compare_two_charArray(db[existEmail].password,tran_pass);
